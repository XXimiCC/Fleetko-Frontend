  ...
  <div class="good" @mouseenter="hovered = true" @mouseleave="hovered = false">
    <div class="good__main">
      <div class="good__image" :class="{ 'good__image--blurred': showNoStock }">
        <router-link :to="{ name: 'product-page', params: { slug: good.slug } }">
          <div v-if="showNoStock" class="good__no-stock">
            <span class="out">Out</span><span class="of-stock">Of Stock</span>
          </div>
          <app-image :imagePath="componentProductImage(good.image)" @emitErrorImage="errorImage">
          </app-image>
        </router-link>
      </div>

      <div class="good__info">
        <router-link class="good__name" :to="{ name: 'product-page', params: { slug: good.slug } }">
          {{ good.name }}
        </router-link>
        ...
        <div class="good__rating">
          <div class="stars">
            ...
            <span class="value">{{ roundDecimalRating(good.rating_average || 0) }}</span>
          </div>
          <router-link class="reviews"
                       :to="{ name: 'product-page', params: { slug: good.slug }, query: { tab: 'reviews' } }">
    ...
    <div class="good__actions">
      <div v-if="!viewedMode" class="reviews">
        <div v-if="good.review" class="review">
          <div class="review__header">
            <div class="review__author">
              ...
              <span itemprop="author">{{ good.review.user.first_name }} {{ good.review.user.last_name }}</span>
            </div>
            <div class="good__rating"
              ...
              <div class="stars">
                ...
                <span class="value">{{ roundDecimalRating(good.review.rating || 0) }}</span>
              </div>
            ...
            <div class="review__date">
              <span itemprop="datePublished">{{ toFormatedDate(good.review.published_at, 'L') }}</span>
            </div>
          </div>

          <div class="review__title">{{ good.review.headline }}</div>

          <p class="review__text" itemprop="description">{{ good.review.text }}</p>
        ...
        <div v-if="!good.review" class="leave">
          <button @click="openModalForCreateReview(good.review)" class="button-prime">
      ...
      <div v-if="viewedMode" class="buy">
        <button v-if="good.quantity && good.price && good.purchasable" @click="openModalCart" class="button-second">
          <svg-basket></svg-basket>
          <span>Add to cart</span>
        </button>

        <button v-if="(!good.quantity || !good.price) && good.status !== 'archived'"
                @click="addToWaitList"
                class="button-second">
          <svg-wishlist v-if="!waitListIncludesProduct"></svg-wishlist>
          <svg-check-xl v-if="waitListIncludesProduct" class="small-svg"></svg-check-xl>
          <span>{{ waitListIncludesProduct ? 'In Wait List' : 'Add to wait list' }}</span>
        </button>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    isPriceGrey () {
      return !this.good.quantity || !this.good.price || this.good.status === 'archived'
    }
  },
  methods: {
    ...
    componentProductImage (img, error) {
      let sizeProperty = ['xl', 'lg', 'md'].includes(this.$mq) ? 'medium' : 'tiny'

      return this.serverImageSource(img, sizeProperty, error, this.SERVER_IMAGE_PRODUCT)
    },
    ...
    openModalCart () {
      this.$store.dispatch('setModalProductId', this.good.slug)
